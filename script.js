// Mengambil data peminjaman dari LocalStorage
let dbPeminjaman = JSON.parse(localStorage.getItem('perpus_data')) || [];

// Master data list novel sesuai permintaanmu
const daftarNovel = [
    "Cerita di Bangku Belakang",
    "Sahabat Sampai Akhir",
    "Kita dan Mimpi-Mimpi",
    "Hari-Hari Paling Indah",
    "Kelas yang Tak Terlupakan",
    "Tentang Kita di Sekolah",
    "Mimpi di Ujung Seragam",
    "Langkah Awal",
...
// ==========================================================================
// 1. FUNGSI NAVIGASI PANEL & INISIALISASI DROPDOWN NOVEL
// ==========================================================================
document.addEventListener("DOMContentLoaded", () => {
    // Jalankan pengisian dropdown novel saat aplikasi pertama kali dimuat
    isiDropdownNovel();
});
...
// Fungsi memasukkan daftar novel ke dalam elemen select HTML
function isiDropdownNovel() {
    const selectBuku = document.getElementById('judul-buku');
    if (!selectBuku) return;

    // Bersihkan isi bawaan terlebih dahulu
    selectBuku.innerHTML = '<option value="">-- Pilih Novel Yang Tersedia --</option>';

    // Loop data array novel dimasukkan menjadi option tag
...
// ==========================================================================
// 2. FUNGSI HITUNG BATAS PENGEMBALIAN BUKU (OTOMATIS 2 HARI)
// ==========================================================================
function hitungBatasKembali() {
    const tgl = document.getElementById('tgl-pinjam').value;
    if (tgl) {
        const d = new Date(tgl);
        d.setDate(d.getDate() + 2); // Batas pinjam otomatis disesuaikan menjadi 2 hari
...
function loginUmum() {
    ...
    // Password pengaman bawaan sistem perpustakaan digital
    if (passInput === 'SMANSALAPERPUS2026##') {
        ...
        // Menampilkan Menu Dashboard Admin khusus rekap log peminjaman
...
// ==========================================================================
// 4. FUNGSI KIRIM PENGAJUAN PINJAM
// ==========================================================================
function mintaPersetujuan() {
    const kelas = document.getElementById('kelas').value;
    const nama = document.getElementById('nama-siswa').value.trim();
    const buku = document.getElementById('judul-buku').value; // Mengambil value pilihan terikat dari dropdown select
    ...
    Swal.fire({
        ...
        document.getElementById('tgl-pinjam').value = '';
...
function renderAdminData() {
    ...
    list.innerHTML = dbPeminjaman.map((d, i) => `
        <tr style="border-bottom: 1px solid rgba(255,255,255,0.05); color: #f8fafc;">   
            ...
            <td style="padding: 10px; color: #93c5fd;">${d.buku}</td>   
            <td style="padding: 10px; font-size:11px; color: #fcd34d;">${d.tglBatas}</td>   
